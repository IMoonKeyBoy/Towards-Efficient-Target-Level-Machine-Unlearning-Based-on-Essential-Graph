    ...
    def compute_cams(
        ...
        with torch.no_grad():
            for weight, activation in zip(weights, self.hook_a):
                ...
                cams.append(cam)
        '''
        weight_list = torch.flatten(weight).tolist()
        import heapq
        max_number = heapq.nlargest(int(len(weight_list) * 0.2), weight_list)
        max_index = []
        for t in max_number:
            index = weight_list.index(t)
            max_index.append(index)
            weight_list[index] = 0
        print(max_index)
        '''
        self.remove_hooks()
        return weight, activation, cams
